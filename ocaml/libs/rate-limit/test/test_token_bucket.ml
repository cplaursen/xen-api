...
let test_bad_fill_rate () =
  ...
  in
  Alcotest.(check bool)
    "Creating a token bucket with 0 fill rate should fail" true
    (raises_invalid_arg (fun () ->
         Token_bucket.create ~burst_size:1.0 ~fill_rate:0.0
     )
    ) ;
  Alcotest.(check bool)
    "Creating a token bucket with negative fill rate should fail" true
    (raises_invalid_arg (fun () ->
         Token_bucket.create ~burst_size:1.0 ~fill_rate:~-.1.0
     )
    )
