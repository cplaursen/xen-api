...
let key s = Caller_table.Key.{user_agent= s; client_ip= ""}
...
let test_insert () =
  let table = Caller_table.create () in
  let success =
    Caller_table.insert table ~caller_id:{user_agent= "test"; client_ip= ""} 42
  in
...
let test_insert_duplicate () =
  let table = Caller_table.create () in
  let success = Caller_table.insert table ~caller_id:(key "agent1") 1 in
  Alcotest.(check bool) "First insert should succeed" true success ;
  let success_dup = Caller_table.insert table ~caller_id:(key "agent1") 2 in
  Alcotest.(check bool) "Inserting duplicate key should fail" false success_dup

let test_delete () =
  let table = Caller_table.create () in
  let _ = Caller_table.insert table ~caller_id:(key "agent1") 42 in
  Alcotest.(check (list int))
    "Entry exists before delete" [42]
    (Caller_table.get table ~caller_id:(key "agent1")) ;
  Caller_table.delete table ~caller_id:(key "agent1") ;
  Alcotest.(check (list int))
...
let test_delete_nonexistent () =
  let table = Caller_table.create () in
  Caller_table.delete table ~caller_id:(key "nonexistent") ;
...
let test_get_nonexistent () =
  ...
  Alcotest.(check (list int))
    ...
    (Caller_table.get table ~caller_id:(key "nonexistent"))

let test_multiple_entries () =
  let table = Caller_table.create () in
  let _ = Caller_table.insert table ~caller_id:(key "agent1") 10 in
  let _ = Caller_table.insert table ~caller_id:(key "agent2") 20 in
  Alcotest.(check (list int))
    "Agent1 has correct value" [10]
    (Caller_table.get table ~caller_id:(key "agent1")) ;
  Alcotest.(check (list int))
    "Agent2 has correct value" [20]
    (Caller_table.get table ~caller_id:(key "agent2"))
        ...
        Thread.create
          (fun () ->
            for i = 0 to iterations - 1 do
              ...
              in
              let _ = Caller_table.insert table ~caller_id:(key k) i in
              ()
            done
          )
          ()
          ...
          (fun () ->
            for i = 0 to iterations - 1 do
              let k =
                Printf.sprintf "key%d" (((t * iterations) + i) mod num_keys)
              in
              Caller_table.delete table ~caller_id:(key k)
            done
          )
          ()
    )
        ...
        Thread.create
          (fun () ->
            for i = 0 to iterations - 1 do
              ...
              try
                let _ = Caller_table.get table ~caller_id:(key k) in
                ()
              with _ ->
...
let test_get_during_delete_race () =
  ...
  for _ = 1 to iterations do
    let table = Caller_table.create () in
    let _ = Caller_table.insert table ~caller_id:(key "target") 42 in
    ...
    let reader =
      Thread.create
        (fun () ->
          ...
          try
            let _ = Caller_table.get table ~caller_id:(key "target") in
            ()
          with _ ->
    ...
    let deleter =
      Thread.create
        (fun () ->
          ...
          Caller_table.delete table ~caller_id:(key "target")
        )
        ()
    in
...
let test_wildcard_user_agent_matches_any () =
  ...
  let table = Caller_table.create () in
  let pattern = Caller_table.Key.{user_agent= "*"; client_ip= "192.168.1.1"} in
  let _ = Caller_table.insert table ~caller_id:pattern 1 in
  (* Should match any user_agent with same client_ip *)
  let client1 =
    Caller_table.Key.{user_agent= "curl"; client_ip= "192.168.1.1"}
  in
  let client2 =
    Caller_table.Key.{user_agent= "wget"; client_ip= "192.168.1.1"}
  in
  let client3 = Caller_table.Key.{user_agent= ""; client_ip= "192.168.1.1"} in
  Alcotest.(check bool)
  ...
  let client_other =
    Caller_table.Key.{user_agent= "curl"; client_ip= "10.0.0.1"}
  in
  Alcotest.(check bool)
...
let test_wildcard_client_ip_matches_any () =
  (* An entry with [*] client_ip should match any client_ip *)
  let table = Caller_table.create () in
  let pattern = Caller_table.Key.{user_agent= "curl"; client_ip= "*"} in
  let _ = Caller_table.insert table ~caller_id:pattern 1 in
  (* Should match any client_ip with same user_agent *)
  let client1 =
    Caller_table.Key.{user_agent= "curl"; client_ip= "192.168.1.1"}
  in
  let client2 = Caller_table.Key.{user_agent= "curl"; client_ip= "10.0.0.1"} in
  let client3 = Caller_table.Key.{user_agent= "curl"; client_ip= ""} in
  Alcotest.(check bool)
...
let test_wildcard_match_priority_exact_first () =
  ...
  let table = Caller_table.create () in
  let exact = Caller_table.Key.{user_agent= "curl"; client_ip= "192.168.1.1"} in
  let wildcard_ua =
    Caller_table.Key.{user_agent= "*"; client_ip= "192.168.1.1"}
  in
  let wildcard_ip = Caller_table.Key.{user_agent= "curl"; client_ip= "*"} in
  (* Add in reverse priority order to test sorting *)
  let _ = Caller_table.insert table ~caller_id:wildcard_ua 5 in
  let _ = Caller_table.insert table ~caller_id:wildcard_ip 15 in
  let _ = Caller_table.insert table ~caller_id:exact 10 in
...
let test_no_spurious_wildcard_matches () =
  ...
  let pattern1 =
    Caller_table.Key.{user_agent= "curl"; client_ip= "192.168.1.1"}
  in
  let pattern2 = Caller_table.Key.{user_agent= "wget"; client_ip= "*"} in
  let _ = Caller_table.insert table ~caller_id:pattern1 10 in
  let _ = Caller_table.insert table ~caller_id:pattern2 20 in
  (* Client with different user_agent and client_ip should not match pattern1 *)
  let client1 = Caller_table.Key.{user_agent= "curl"; client_ip= "10.0.0.1"} in
  Alcotest.(check bool)
  ...
  let client2 = Caller_table.Key.{user_agent= "wget"; client_ip= "10.0.0.1"} in
  Alcotest.(check (list int))
  ...
  let client3 =
    Caller_table.Key.{user_agent= "firefox"; client_ip= "172.16.0.1"}
  in
  Alcotest.(check bool)
...
let test_lru_cache_overflow () =
  ...
  let table = Caller_table.create () in
  let pattern = Caller_table.Key.{user_agent= "curl"; client_ip= "*"} in
  let _ = Caller_table.insert table ~caller_id:pattern 42 in
  ...
  let miss = Caller_table.Key.{user_agent= "wget"; client_ip= "10.0.0.1"} in
  Alcotest.(check (list int))
...
let test_reject_all_wildcard_key () =
  ...
  let table = Caller_table.create () in
  let all_wildcard = Caller_table.Key.{user_agent= "*"; client_ip= "*"} in
  let success = Caller_table.insert table ~caller_id:all_wildcard 1 in
  Alcotest.(check bool) "all-wildcard key rejected" false success

let test_prefix_wildcard_matches_prefix () =
  let table = Caller_table.create () in
  let pattern =
    Caller_table.Key.{user_agent= "xen_api_libs/*"; client_ip= "192.168.1.1"}
  in
  let _ = Caller_table.insert table ~caller_id:pattern 7 in
  let matching =
    Caller_table.Key.{user_agent= "xen_api_libs/1.2"; client_ip= "192.168.1.1"}
  in
  let non_matching =
    Caller_table.Key.{user_agent= "xen_api/1.2"; client_ip= "192.168.1.1"}
  in
  Alcotest.(check (list int))
...
let test_prefix_priority_over_full_wildcard () =
  let table = Caller_table.create () in
  let full = Caller_table.Key.{user_agent= "*"; client_ip= "192.168.1.1"} in
  let prefix =
    Caller_table.Key.{user_agent= "xen_api_libs/*"; client_ip= "192.168.1.1"}
  in
  let _ = Caller_table.insert table ~caller_id:full 5 in
  let _ = Caller_table.insert table ~caller_id:prefix 9 in
  let client =
    Caller_table.Key.{user_agent= "xen_api_libs/1.2"; client_ip= "192.168.1.1"}
  in
  Alcotest.(check (list int))
...
let test_get_ordered_specificity_ladder () =
  ...
  let table = Caller_table.create () in
  let exact =
    Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "192.168.1.1"}
  in
  let ua_exact_ip_prefix =
    Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "192.168.*"}
  in
  let ua_prefix_ip_exact =
    Caller_table.Key.{user_agent= "curl/*"; client_ip= "192.168.1.1"}
  in
  let ua_exact_ip_full =
    Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "*"}
  in
  let ua_prefix_ip_prefix =
    Caller_table.Key.{user_agent= "curl/*"; client_ip= "192.168.*"}
  in
  let ua_full_ip_exact =
    Caller_table.Key.{user_agent= "*"; client_ip= "192.168.1.1"}
  in
  let ua_prefix_ip_full =
    Caller_table.Key.{user_agent= "curl/*"; client_ip= "*"}
  in
  let ua_full_ip_prefix =
    Caller_table.Key.{user_agent= "*"; client_ip= "192.168.*"}
  in
  (* Insert in a scrambled order to exercise the sorted-insert logic *)
  let _ = Caller_table.insert table ~caller_id:ua_prefix_ip_full 7 in
  let _ = Caller_table.insert table ~caller_id:exact 1 in
  let _ = Caller_table.insert table ~caller_id:ua_full_ip_prefix 8 in
  let _ = Caller_table.insert table ~caller_id:ua_prefix_ip_exact 3 in
  let _ = Caller_table.insert table ~caller_id:ua_exact_ip_full 4 in
  let _ = Caller_table.insert table ~caller_id:ua_full_ip_exact 6 in
  let _ = Caller_table.insert table ~caller_id:ua_exact_ip_prefix 2 in
  let _ = Caller_table.insert table ~caller_id:ua_prefix_ip_prefix 5 in
  let client =
    Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "192.168.1.1"}
  in
  Alcotest.(check (list int))
...
let test_get_ordered_tiebreak_lex () =
  ...
  let table = Caller_table.create () in
  let p_a = Caller_table.Key.{user_agent= "a*"; client_ip= "10.0.0.1"} in
  let p_ab = Caller_table.Key.{user_agent= "ab*"; client_ip= "10.0.0.1"} in
  let p_abc = Caller_table.Key.{user_agent= "abc*"; client_ip= "10.0.0.1"} in
  (* Insert in reverse lex order *)
  let _ = Caller_table.insert table ~caller_id:p_abc 30 in
  let _ = Caller_table.insert table ~caller_id:p_ab 20 in
  let _ = Caller_table.insert table ~caller_id:p_a 10 in
  let client =
    Caller_table.Key.{user_agent= "abc/foo"; client_ip= "10.0.0.1"}
  in
  (* Lex order: "a*" < "ab*" < "abc*" *)
  Alcotest.(check (list int))
...
let test_get_ordered_tiebreak_client_ip () =
  (* Ties on user_agent fall through to client_ip lex order. Patterns
     ...
     decides. *)
  let table = Caller_table.create () in
  let p1 = Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "10.0.*"} in
  let p2 = Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "10.0.0.*"} in
  let _ = Caller_table.insert table ~caller_id:p2 200 in
  let _ = Caller_table.insert table ~caller_id:p1 100 in
  let client =
    Caller_table.Key.{user_agent= "curl/1.2"; client_ip= "10.0.0.5"}
  in
  Alcotest.(check (list int))
...
let test_get_ordered_after_delete_reinsert () =
  ...
  let table = Caller_table.create () in
  let exact = Caller_table.Key.{user_agent= "curl"; client_ip= "1.2.3.4"} in
  let ua_full = Caller_table.Key.{user_agent= "*"; client_ip= "1.2.3.4"} in
  let ip_full = Caller_table.Key.{user_agent= "curl"; client_ip= "*"} in
  let _ = Caller_table.insert table ~caller_id:ua_full 2 in
  let _ = Caller_table.insert table ~caller_id:exact 1 in
  let _ = Caller_table.insert table ~caller_id:ip_full 3 in
  let client = Caller_table.Key.{user_agent= "curl"; client_ip= "1.2.3.4"} in
  Alcotest.(check (list int))
    "ordered before delete" [1; 3; 2]
    (Caller_table.get table ~caller_id:client) ;
  Caller_table.delete table ~caller_id:ip_full ;
  Alcotest.(check (list int))
    "ordered after delete" [1; 2]
    (Caller_table.get table ~caller_id:client) ;
  let _ = Caller_table.insert table ~caller_id:ip_full 30 in
  Alcotest.(check (list int))
