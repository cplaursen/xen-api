...
let check_stats msg ~call_count ~tokens_consumed stats =
  Alcotest.(check int) (msg ^ ": call_count") call_count stats.Caller.call_count ;
  Alcotest.(check (float 0.001))
    (msg ^ ": tokens_consumed")
    tokens_consumed stats.Caller.tokens_consumed
...
let test_sliding_window_prunes_old_calls () =
  ...
  Thread.delay 0.3 ;
  let stats_after = Option.get (Caller.get_stats t ~client_id:id) in
...
let test_set_rate_limiter_nonexistent () =
  ...
  Caller.remove_rate_limiter t ~client_id
